...
prevalence_dict_path = '%svotu_prevalence_dict.pickle' % config.data_directory
...
def plot_prevalence():
    ...
    # get phage presence/absence dictionary
    votu_dict, vgenome_dict = data_utils.read_uhgv_metadata(checkv_quality='Complete', checkv_quality_cumulative=True)
    # build the presence/absence matrix
    votu_pres_abs_array, votu_names, sample_names = data_utils.votu_dict_to_pres_abs(votu_dict)
    ...
    for ranks_chunk_idx, ranks_chunk in enumerate(ranks_chunk_all):
        ...
        for ranks_idx, ranks in enumerate(ranks_chunk):
            ...
            for lifestyle in data_utils.lifestyle_all_and_both:
                ...
                survival_array = stats_utils.make_survival_dist(prevalence, prevalence_range)

                ax.plot(prevalence_range, survival_array, lw=2, ls='-', c=plot_utils.lifestyle_color_dict[lifestyle], label=lifestyle.capitalize())
            ...
            statistic, p_value = stats_utils.permutation_two_sample_ks_test(prevalence_lytic, prevalence_temperate)

            #print(statistic, p_value )

            pbalue_label = plot_utils.get_latex_pvalue(p_value)
